...
async function adminFetchAll(){
  ...
  const users=new Map();
  try{ const snap=await cloud.db.collection("leaderboard_pruebas").get(); snap.forEach(d=>{ const x=d.data()||{}; users.set(d.id,{uid:d.id, nick:x.nick||"", city:x.city||""}); }); }catch(e){}
  try{ const asnap=await cloud.db.collection("albums_pruebas").get(); asnap.forEach(d=>{ if(!users.has(d.id)) users.set(d.id,{uid:d.id, nick:"", city:""}); }); }catch(e){}
  if(!users.size) throw new Error("No hay álbumes en la nube todavía");
  const out=[];
  for(const u of users.values()){
    try{ const doc=await cloud.db.collection("albums_pruebas").doc(u.uid).get(); const data=doc.exists?(doc.data()||{}):{};
...
function pushLeaderboard(){
  ...
  const s=leaderStats();
  return cloud.db.collection("leaderboard_pruebas").doc(cloud.user.uid).set({ nick:nick, city:city, pct:s.pct, tengo:s.tengo, total:s.total, updatedAt:Date.now() }).then(()=>true);
}
function leaveLeaderboard(){
  if(!cloud.ready||!cloud.user||!cloud.db) return Promise.resolve();
  return cloud.db.collection("leaderboard_pruebas").doc(cloud.user.uid).delete().catch(()=>{});
}
function renderRanking(){
  ...
  Promise.resolve(pushLeaderboard()).catch(()=>null).then(()=> cloud.db.collection("leaderboard_pruebas").get() ).then(snap=>{
...
function subscribeAlbum(){
  ...
  applyViewOnly();
  const ref=cloud.db.collection("albums_pruebas").doc(cloud.albumId);
...
function pushCloud(seed){
  ...
  cloud.db.collection("albums_pruebas").doc(id).set({ counts:state.counts, extraNames:state.config.extraNames, specialPages:state.config.specialPages||{}, groupPages:state.config.groupPages||{}, nick:state.config.nick||"", city:state.config.city||"", updatedAt:Date.now() }).catch(e=>{ if(!seed) toast("Nube: error al guardar"); });
  pushLeaderboard();
}
